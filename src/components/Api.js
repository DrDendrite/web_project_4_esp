export default class Api {
    ...
    async getUserInfoFromServer() {
      try {
        const res = await this._useFetch(
          "https://around.nomoreparties.co/v1/web_es_10/users/me",
          "GET"
        );
        return res;
      } catch (err) {
    ...
    async deleteCardFromServer(cardId) {
      try {
        const res = await this._useFetch(
          `https://around.nomoreparties.co/v1/web_es_10/cards/${cardId}`,
          "DELETE"
        );
    ...
    async updateAvatar(avatarUrl) {
      try {
        const res = await this._useFetch(
          "https://around.nomoreparties.co/v1/web_es_10/users/me/avatar",
          "PATCH",
          {
            avatar: avatarUrl,
          }
